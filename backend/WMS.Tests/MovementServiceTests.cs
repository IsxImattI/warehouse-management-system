...
public class MovementServiceTests
{
    ...
    public async Task Move_IncomingStock_CreatesInventoryRecord()
    {
        ...
        await svc.ExecuteAsync(new MoveRequest(item.Id, null, locA.Id, 10, "Initial stock"));
    ...
    public async Task Move_BetweenLocations_UpdatesBothInventories()
    {
        ...
        // First incoming
        await svc.ExecuteAsync(new MoveRequest(item.Id, null, locA.Id, 10, null));

        // Move 4 units to locB
        await svc.ExecuteAsync(new MoveRequest(item.Id, locA.Id, locB.Id, 4, null));
    ...
    public async Task Move_CreatesMovementLog()
    {
        ...
        await svc.ExecuteAsync(new MoveRequest(item.Id, null, locA.Id, 5, "Test log"));
    ...
    public async Task Move_InsufficientStock_ThrowsException()
    {
        ...
        await svc.ExecuteAsync(new MoveRequest(item.Id, null, locA.Id, 5, null));

        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            svc.ExecuteAsync(new MoveRequest(item.Id, locA.Id, locB.Id, 99, null)));
    }
    ...
    public async Task Move_ItemNotAtSourceLocation_ThrowsException()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            svc.ExecuteAsync(new MoveRequest(item.Id, locA.Id, locB.Id, 5, null)));
    }
}